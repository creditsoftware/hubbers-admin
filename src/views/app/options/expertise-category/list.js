...
const ExpertiseCategoryList = () => {
  ...
  const getExpertiseSubcategory = (parentId) => {
    ...
    return <Table
      ...
      dataSource={expertiseCategoryList.filter((item) => item.parentId === parentId)}
      expandable={{
  ...
  return (
    <Card>
      <div className="text-right mb-3">
        <CreateExpertiseCategory type="expertiseCategory" buttonContent="Create New ExpertiseCategory" />
      </div>
      <div className="table-responsive">
        <Table
          rowKey="id"
          columns={tableColumns}
          dataSource={expertiseCategoryList?.filter((item) => item.categoryType === "expertiseCategory")}
          expandable={{
